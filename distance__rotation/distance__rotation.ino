...
void setup() {
  ...
  attachInterrupt(3, rencoder_1, FALLING);
...
void loop() {
  ...
  if((millis()-lastMilli) >= LOOPTIME)   {                                    // enter tmed loop
    ...
    PWM_1= updatePid_1(PWM_1, speed_req, speed_act_1);
    PWM_2= updatePid_2(PWM_2, speed_req, speed_act_2);    // compute PWM value
//    analogWrite(PWM1, PWM_1);
//    analogWrite(PWM2, PWM_2);
//    motorForawrd();
//    delay(1000);
...
void motorTurn(){
  count_1 = 0;
  while (count_1 < 7600){
    analogWrite(PWM1, PWM_1/2);
    analogWrite(PWM2, PWM_2/2);
...
void printMotorInfo()  {                                                      // display data
  if((millis()-lastMilliPrint) >= 500)   {                     
    ...
    Serial.println(count_1);             //Serial.print(speed_req);
