...
static int moveTurns = 200;
static int moveConst = 0;
...
void setup() {
  ...
  pinMode(encodPinB2, INPUT);             // turn on pullup resistor
...
void rencoder_1()  {                                    // pulse and direction, direct port reading to save cycles
  if (digitalRead(encodPinA1)==LOW)    count_1++;                // if(digitalRead(encodPinB1)==HIGH)   count ++;
